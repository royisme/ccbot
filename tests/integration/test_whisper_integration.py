...
class TestWhisperConfigIntegration:
    ...
    def test_whisper_fields_from_env(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CCBOT_WHISPER_PROVIDER", "groq")
        monkeypatch.setenv("CCBOT_WHISPER_MODEL", "whisper-large-v3-turbo")
        monkeypatch.setenv("CCBOT_WHISPER_LANGUAGE", "zh")
    ...
    def test_whisper_fields_from_dotenv(self, tmp_path, monkeypatch):
        ...
        env_file.write_text(
            ...
            "ALLOWED_USERS=1\n"
            "CCBOT_WHISPER_PROVIDER=openai\n"
        )
        monkeypatch.setenv("CCBOT_DIR", str(tmp_path))
        monkeypatch.delenv("TELEGRAM_BOT_TOKEN", raising=False)
        monkeypatch.delenv("CCBOT_WHISPER_PROVIDER", raising=False)
    ...
    def test_custom_api_key_field(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("TELEGRAM_BOT_TOKEN", "tok-test")
        monkeypatch.setenv("ALLOWED_USERS", "1")
        monkeypatch.setenv("CCBOT_WHISPER_API_KEY", "custom-override-key")
...
class TestGetTranscriberIntegration:
    ...
    def test_returns_none_when_provider_empty(self, monkeypatch):
        ...
        from ccbot.whisper import get_transcriber
    ...
    def test_returns_openai_transcriber(self, monkeypatch):
        ...
        monkeypatch.setattr("ccbot.config.config.whisper_base_url", "")
        monkeypatch.setattr("ccbot.config.config.whisper_model", "")
    ...
    def test_model_override_applies(self, monkeypatch):
        ...
        monkeypatch.setattr("ccbot.config.config.whisper_base_url", "")
        monkeypatch.setattr(
            "ccbot.config.config.whisper_model", "whisper-large-v3-turbo"
        )
        monkeypatch.setenv("GROQ_API_KEY", "gsk-test")
    ...
    def test_custom_api_key_takes_precedence_over_env(self, monkeypatch):
        monkeypatch.setattr("ccbot.config.config.whisper_provider", "groq")
        monkeypatch.setattr("ccbot.config.config.whisper_api_key", "custom-key")
        monkeypatch.setattr("ccbot.config.config.whisper_base_url", "")
        monkeypatch.setattr("ccbot.config.config.whisper_model", "")
    ...
    def test_raises_on_unknown_provider(self, monkeypatch):
        monkeypatch.setattr("ccbot.config.config.whisper_provider", "unknown-llm")
